...
import ProForm, { BetaSchemaForm, DrawerForm, DrawerFormProps, ProFormColumnsType, ModalForm, ModalFormProps } from '@ant-design/pro-form';
...
const myUtils = {
     ...
     */
    getChangesParams: (row: any, theOriginRow: any, addRowkey?: { key: string }) => {
     ...
     */
    customFormElement: (theForms: CustomForm[], formProps: ModalFormProps | DrawerFormProps, formType: 'DrawerForm' | 'ModalForm') => (baseProps: comptsBaseProps) => {

        const [form] = Form.useForm();

        const children = theForms.map((e, index) => {
            if (e.renderFormItem) {
                return <ProForm.Item {...e.renderFormItem} key={index}>
                    {e.compt(form)}
                </ProForm.Item>
            }
            else {
        ...
        return <TheForm {...formProps} form={form}>{children}</TheForm>;
    },
    ...
    addTrigger: (props: postProps) => (baseProps: comptsBaseProps) => {
        ...
        if (forms.length === 1) {
            ...
            //jsong格式
            if (forms[0].type === "json") return <BetaSchemaForm
                trigger={<Tooltip title={`添加新的${title}`}><a className="anticon" style={{ fontSize: '17px' }}><PlusOutlined /></a></Tooltip>}
            ...
            if (forms[0].type === "custom") {
                const TheForm = myUtils.customFormElement(columns as CustomForm[], {
                    trigger: <Tooltip title={`添加新的${title}`}><a className="anticon" style={{ fontSize: '17px' }}><PlusOutlined /></a></Tooltip>,
                    onFinish,
                    title: `添加新的${title}`,
                    width: width ?? 500
            ...
            const theBodyForms = forms.map((e, index) => {
                ...
                if (type === "json") {
                    return <BetaSchemaForm
                        trigger={<Button type="dashed" block style={{ marginTop: 10 }} size="large">{title}</Button>}
                    ...
                    const TheCustomForm = myUtils.customFormElement(columns as CustomForm[], {
                        trigger: <Button type="dashed" block style={{ marginTop: 10 }} size="large">{title}</Button>,
                        onFinish: async (values) => {
                            const result = await onFinish(values);
                            if (result === true) {
            ...
            return <>
                <Tooltip title={`添加新的${props.title}`}><a className="anticon" style={{ fontSize: '17px' }} onClick={() => setVisible(true)}><PlusOutlined /></a></Tooltip>
                <Drawer
     ...
     */
    deleteTrigger: (props: deleteProps, row: any, action: ActionType | undefined) => (baseProps: comptsBaseProps) => {

        const { content, onFinish, triggerText } = props;
     ...
     */
    updateTrigger: (props: updateProps, theRowKeyValue: string, action: ActionType | undefined, columns: any[], editRow: object, theRowKey?: string) => (baseProps: comptsBaseProps) => {

        const { type, isPatch, onFinish, triggerText } = props;
        ...
        if (type === "line") return <a onClick={lineClick}>{theTriggerText}</a>
            ...
            return <BetaSchemaForm
                ...
                onFinish={(values: object) => {
                    ...
                    let originData: any = { [theRowKey ?? "id"]: theRowKeyValue, ...values };

                    let result = originData;
                    if (isPatch) {
                        if (isPatch !== true && isPatch.addKey) result = myUtils.getChangesParams(originData, editRow, { key: theRowKey ?? "id" });
