...
import type {
    comptsBaseProps, CustomForm, formsSingle, postProps,
    deleteProps, updateProps, cudProps, TuyeeTableProps
} from './types';
...
const tuyeeTable: React.FC<TuyeeTableProps> = (originProps) => {
    ...
    const { toolBarRender, cudProps, headerTitle, columns } = originProps;
    ...
    useEffect(() => {
        ...
        tableProps.scroll = { scrollToFirstRowOnChange: true, x: true };

        //計算 toolBarRender 是否有必要渲染
        const isNeedToolBar = (cudProps?.post !== undefined) || (toolBarRender !== undefined) || (headerTitle !== undefined);
            ...
            if (type === "line") {
                tableProps.editable = {
                    ...
                    onSave: (key, changes) => {
                        let data = changes;
                        if (isPatch) {
                            if (isPatch !== true && isPatch.addKey) data = myUtils.getChangesParams(changes, editRows[0], { key: tableProps.rowKey as string });
        ...
        if (columns) {
            ...
            if (originOptions.length > 0 || (cudProps && cudProps.update) || (cudProps && cudProps.delete)) {
                tableProps.columns.push({
                    ...
                    render: (dom, row, index, action, schema) => {
                        ...
                        for (const e of originOptions) {
                            if (e.render) result.push(e.render(dom, row, index, action, schema));
                        }

                        if (cudProps && cudProps.update) {
                            const TheUpdate = myUtils.updateTrigger(cudProps.update, row[tableProps.rowKey as string], action, columns, row, tableProps.rowKey as string);
                            result.push(<TheUpdate key={"update" + index} />);
                        }

                        if (cudProps && cudProps.delete) {
                            const TheDelete = myUtils.deleteTrigger(cudProps.delete, row, action);
...
export type { comptsBaseProps, CustomForm, formsSingle, postProps, deleteProps, updateProps, cudProps, TuyeeTableProps };
